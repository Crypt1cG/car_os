...
public:
    ...
    // my addition (make return bool?):
    void connect(const char* addr_string);
...
private:
    ...
    // had to make this not const
    int RFCOMM_SERVER_CHANNEL = 1;
    ...
    // added by me
    bd_addr_t _peer_addr;
    btstack_context_callback_registration_t _handle_sdp_client_query_request;
    static void handle_start_sdp_client_query_wrapper(void* context) {
        SerialBT.handle_start_sdp_client_query(context);
    }
    void handle_start_sdp_client_query(void* context);
    static void handle_query_rfcomm_event_wrapper(uint8_t packet_type, uint16_t channel, uint8_t* packet, uint16_t size) {
        SerialBT.handle_query_rfcomm_event(packet_type, channel, packet, size);
    }
